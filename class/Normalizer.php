...
class Normalizer
{
    ...
    static function normalize($s, $form = self::FORM_C)
    {
        ...
        if ($K && empty(self::$KD)) self::$KD = unserialize(file_get_contents(patchworkPath('data/utf8/compatibilityDecomposition.ser')));

        $s = self::decompose($s, $K);

        return $C ? self::recompose($s) : $s;
    }
